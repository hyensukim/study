...
- call, apply를 이용한 간접 호출
    ...
    - Functin.prototype.call() : 주어진 this값 및 각각 전달된 인수와 함께 호출됨.
- 즉시 실행 함수  
**`(function() {...})();`**
...
## 프로그램 평가와 실행 과정
- 자바 스크립트에서는 함수가 객체이므로 코드 자체가 실행되는 것이
아니라, 실행문맥이라는 객체를 생성하여 재구성된다.
...
## 자바스크립트는 싱글 쓰레드.
- 자바스크립트 초기에 웹에서 비중이 작은 언어 이었으며, 이로 인해 싱글 스레드 방식으로 사용되었다. 하지만 점차 비중이 커지게 되면서 좀 더 발전된 방식을 추구하게 된다.
